...
| `DASHBOARD_COLUMNS_PINNED` | `lanes:LANE,claude:WINDOW` | columns that never scroll off and are never squeezed, same shape. Unset is the default shown here — the column that names each row; an explicit empty value pins nothing |
