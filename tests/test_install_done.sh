...
check "and names install.sh to re-run" grep -qF "run $REPO/install.sh again" <<<"$(after_done "$out")"
...
check "nothing said it cannot run"      bash -c '! grep -q "cannot run yet" <<<"$1"' _ "$out"

echo "== claude on PATH: nothing to say"
out="$(PATH="$BIN:$PATH" inst)"
check "step 1: + claude"                grep -q '+ claude$' <<<"$out"
check "no PATH line for this shell"     bash -c '! grep -q "For this shell" <<<"$1"' _ "$out"
